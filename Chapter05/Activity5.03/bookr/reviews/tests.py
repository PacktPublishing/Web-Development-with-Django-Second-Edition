...
class Activity3Test(StaticLiveServerTestCase):
    ...
    def test_reviews_logo_get(self):
        ...
        with open(
            os.path.join(current_dir, "static", "reviews", "logo.png"), "rb"
        ) as f:
    ...
    def test_base_logo_display(self):
        ...
        resp = c.get("/")
        self.assertIn(
            b'<a class="navbar-brand" href="/"><img src="/static/logo.png"></a>',
            resp.content,
        )
    ...
    def test_reviews_logo_display(self):
        ...
        resp = c.get("/books/")
        self.assertIn(
            b'<a class="navbar-brand" href="/"><img src="/static/reviews/logo.png"></a>',
            resp.content,
        )

    def test_review_base_extends_main(self):
        """Test that the reviews base.html extends from the main base.html"""
        reviews_base = read_content(
            os.path.join(
                settings.BASE_DIR, "reviews", "templates", "reviews", "base.html"
            )
        )
    ...
    def test_reviews_templates_extend_reviews_base(self):
        ...
        book_detail_template = read_content(
            os.path.join(
                settings.BASE_DIR, "reviews", "templates", "reviews", "book_detail.html"
            )
        )
        self.assertTrue(
            book_detail_template.startswith("{% extends 'reviews/base.html' %}")
        )

        book_list_template = read_content(
            os.path.join(
                settings.BASE_DIR, "reviews", "templates", "reviews", "book_list.html"
            )
        )
        self.assertTrue(
            book_list_template.startswith("{% extends 'reviews/base.html' %}")
        )
