...
class Activity1Test(TestCase):
    ...
    def setUpTestData(cls):
        ...
        b = Book.objects.create(
            title="Test Book", publication_date=timezone.now(), publisher=p
        )
    ...
    def test_form_in_template(self):
        ...
        self.assertIn(
            b'<input type="file" name="cover" accept="image/*" id="id_cover">',
            resp.content,
        )
        self.assertIn(b'<input type="file" name="sample" id="id_sample">', resp.content)
    ...
    def test_render_call(self, mock_g_o_o_404, mock_render):
        ...
        self.assertEquals(
            mock_render.call_args[0][2]["instance"], mock_g_o_o_404.return_value
        )
    ...
    def test_book_media_upload(self):
        ...
        cover_save_path = os.path.join(
            settings.MEDIA_ROOT, "book_covers", cover_filename
        )

        sample_filename = "machine-learning-for-trading.pdf"
        sample_save_path = os.path.join(
            settings.MEDIA_ROOT, "book_samples", sample_filename
        )

        try:
            c = Client()
            with open(
                os.path.join(settings.BASE_DIR, "fixtures", cover_filename), "rb"
            ) as cover_fp:
                with open(
                    os.path.join(settings.BASE_DIR, "fixtures", sample_filename), "rb"
                ) as sample_fp:
                    resp = c.post(
