...
class Exercise6Test(LiveServerTestCase):
    ...
    def test_form_in_template(self):
        ...
        self.assertIn(
            b'<label for="id_image_upload">Image upload:</label>', resp.content
        )
        self.assertIn(
            b'<input type="file" name="image_upload" accept="image/*" required id="id_image_upload">',
            resp.content,
        )
        self.assertIn(
            b'<input type="file" name="file_upload" required id="id_file_upload">',
            resp.content,
        )
    ...
    def test_media_example_upload(self):
        ...
        try:
            c = Client()
            with open(
                os.path.join(settings.BASE_DIR, "fixtures", logo_filename), "rb"
            ) as logo_fp:
                with open(
                    os.path.join(settings.BASE_DIR, "fixtures", css_filename), "rb"
                ) as css_fp:
                    resp = c.post(
                        "/media-example/",
                        {"file_upload": css_fp, "image_upload": logo_fp},
                    )
