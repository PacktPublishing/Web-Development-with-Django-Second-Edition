...
class Exercise2Test(TestCase):
    def test_fields_in_view(self):
        ...
        self.assertIn(
            '<p><label for="id_magazine_count">Magazine count:</label><input type="number" '
            'name="magazine_count" placeholder="Number of Magazines" min="0" max="80" required '
            'id="id_magazine_count"></p>',
            content,
        )
        self.assertIn(
            '<p><label for="id_book_count">Book count:</label><input type="number" name="book_count" '
            'placeholder="Number of Books" min="0" max="50" required id="id_book_count"></p>',
            content,
        )
        self.assertIn(
            '<p><label for="id_send_confirmation">Send confirmation:</label><input type="checkbox" '
            'name="send_confirmation" id="id_send_confirmation"></p>',
            content,
        )
        self.assertIn(
            '<p><label for="id_email">Email:</label><input type="email" name="email" value="user@example.com" '
            'placeholder="Your company email address" id="id_email"></p>',
            content,
        )

        self.assertIn(
            '<input type="submit" name="submit_input" value="Submit Input">', content
        )
        self.assertIn(
    ...
    def test_order_form_quantity_exceeded(self):
        ...
        self.assertEqual(
            form.non_field_errors(), ["The total number of items must be 100 or less."]
        )
    ...
    def test_email_required_on_send_confirmation(self):
        ...
        form = OrderForm(
            {"magazine_count": "80", "book_count": "50", "send_confirmation": "on"}
        )
