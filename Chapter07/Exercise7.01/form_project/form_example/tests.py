...
class Exercise1Test(TestCase):
    def test_fields_in_view(self):
        ...
        self.assertIn('<p><label for="id_magazine_count">Magazine count:</label><input type="number" '
                      'name="magazine_count" min="0" max="80" required id="id_magazine_count"></p>', content)
        self.assertIn('<p><label for="id_book_count">Book count:</label><input type="number" name="book_count" '
                      'min="0" max="50" required id="id_book_count"></p>', content)
        self.assertIn('<p><label for="id_send_confirmation">Send confirmation:</label><input type="checkbox" '
                      'name="send_confirmation" id="id_send_confirmation"></p>', content)
        self.assertIn(
            '<p><label for="id_email">Email:</label><input type="email" name="email" id="id_email"></p>',
            content)

        self.assertIn('<input type="submit" name="submit_input" value="Submit Input">', content)
        self.assertIn('<button type="submit" name="button_element" value="Button Element">', content)
    ...
    def test_post_debug_output(self, mock_print):
        ...
        mock_request.POST = QueryDict(
            'csrfmiddlewaretoken=B0Bda7HRlbhqnREQYbonOzMKgAGFjn3gBT7HXIyq5FSnBFK5iWOz73ucgl3jKtUv&magazine_count=30&'
            'book_count=20&send_confirmation=on&email=MyEMail%40example.com&submit_input=Submit+Input'
        )
        mock_request.META = {}
        form_example(mock_request)
