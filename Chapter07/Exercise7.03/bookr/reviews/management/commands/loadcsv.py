...
class Command(BaseCommand):
    ...
    def get_or_create_user(self, data_dict):
        try:
            creator = User.objects.get(
                email=data_dict["review_creator_email"],
                username=data_dict["review_creator_username"],
            )
        ...
        try:
            creator = User.objects.get(username=data_dict["review_creator_username"])
            return creator, False
        except User.DoesNotExist as e:
            pass
    ...
    def handle(self, *args, **options):
        ...
        for data_dict in models.get("Review", []):
            creator, created = self.get_or_create_user(data_dict)

            if created:
                print(
                    'Created User "{}". Password is: {}'.format(
