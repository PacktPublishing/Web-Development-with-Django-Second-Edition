...
class Activity2Test(TestCase):
    ...
    def test_fields_and_labels_in_view(self):
        ...
        self.assertIn(
            '<label for="id_content">Content:</label><textarea name="content" cols="40" rows="10" '
            'required id="id_content"></textarea><span class="helptext">The Review text.</span>',
            content,
        )
        self.assertIn(
            '<label for="id_rating">Rating:</label><input type="number" name="rating" min="0" max="5" required '
            'id="id_rating">',
            content,
        )
        self.assertIn('<label for="id_creator">Creator:</label>', content)
        self.assertIn('<select name="creator" required id="id_creator">', content)
        self.assertIn('<option value="" selected>---------</option>', content)
        self.assertIn('<option value="1">testuser</option>', content)
        self.assertIn('<option value="2">testuser2</option>', content)
        self.assertNotIn(
            'name="date_edited"', content
        ...
        self.assertIn("<p>For Book <em>Test Book (123456)</em></p>", content)
